...
class MyRobot:
    ...
    def move_end_effector_to_position(self, x, y, z):
	    # Set the target pose in Cartesian space
	    target_pose = geometry_msgs.msg.Pose()
	    target_pose.position.x = x
	    target_pose.position.y = y
	    target_pose.position.z = z
	    target_pose.orientation.w = 1.0  # Identity orientation (no rotation)

	    # Convert the target pose to a list
	    target_pose_list = pose_to_list(target_pose)

	    # Plan to the target pose with a tolerance of 0.01 meters and 0.1 radians
	    self._group.set_pose_target(target_pose_list, self._eef_link)
	    plan_success, plan, planning_time, error_code = self._group.plan()
	    self._move_group.set_planning_time(5)  # Set planning time to 5 seconds
	    # If planning was successful, execute the motion
	    if plan_success:
	       self._exectute_trajectory_client.send_goal(moveit_msgs.msg.ExecuteTrajectoryGoal(trajectory=plan))
	       self._exectute_trajectory_client.wait_for_result()
	       rospy.loginfo('\033[32m' + "Now at position: ({}, {}, {})".format(x, y, z) + '\033[0m')
	    else:
	       rospy.logerr("Planning to the target position failed with error code: {}".format(error_code))
